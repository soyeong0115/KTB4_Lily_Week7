...
### 직접 구현(알림 페이지)에서 어려웠던 점

직접 구현하면서 가장 손이 많이 간 부분은 무엇을 어디에 그릴지를 스스로 계산하는 것이었다. `scrollTop`으로 `startIndex`를 구하고, 잘라낸 아이템을 원래 위치(`top = index * ITEM_HEIGHT`)에 절대 배치하는 로직에서, 슬라이스한 배열의 인덱스(`i`)와 전체 배열의 인덱스(`startIndex + i`)를 헷갈리기 쉬웠다. overscan을 붙일 때는 `startIndex`에서 이미 뺀 값을 `endIndex` 계산에 다시 더해 상쇄돼버리는 버그를 겪었는데, 코드만 봐서는 안 보이고 실제 숫자를 대입해봐야 드러났다. 성능 최적화도 직접 챙겨야 했다. rAF 쓰로틀링은 효과가 없었고(중복 렌더가 애초에 없었기 때문에), `React.memo`를 붙여서야 스크롤 중 비용이 눈에 띄게 줄었다. 즉 windowing의 원리(위치 계산, overscan, 리렌더 최소화)를 하나하나 직접 부딪혀야 했고, 그만큼 왜 이런 게 필요한지는 확실히 이해할 수 있었다.

### 라이브러리(게시글 목록)로 구현하면서 편했던 점과 그래도 직접 해야 했던 점

react-window는 세로 리스트라는 정해진 틀 안에서는 매우 편했지만, 게시글 목록의 3단 masonry처럼 그 틀을 벗어나는 레이아웃은 아예 표현하지 못했다. 라이브러리를 쓰면 다 되는 게 아니라, 그 라이브러리가 전제하는 구조에 내 화면이 맞아야 한다는 걸 이때 배웠다.

TanStack Virtual로 옮기고 나서는, 직접 구현할 때 어려움을 느꼈던 위치 계산·overscan·리렌더 최소화를 라이브러리가 대신 해줬다. 특히 `lanes` 옵션이 열 분배(어느 카드가 몇 번 열, 세로 위치 얼마)까지 계산해줘서 masonry가 그대로 유지됐다. 다만 react-window처럼 컴포넌트에 넘기면 끝은 아니었고, 계산 결과(`lane`, `start`)를 받아 실제 DOM 배치와 컨테이너 폭 측정, 무한스크롤 연결은 직접 해야 했다. 직접 구현과 완전 자동 라이브러리의 중간쯤이라는 느낌이 들었다.

### 언제 직접 짜고 언제 라이브러리를 쓸까

직접 구현은 원리를 이해하고 완전히 통제할 수 있다는 장점이 있지만, overscan·rAF·memo 같은 최적화를 전부 스스로 챙겨야 하고 그 과정에서 실수하기 쉬웠다. 라이브러리는 이런 검증된 최적화가 이미 들어 있어 빠르고 안정적이지만, 그 라이브러리가 지원하는 레이아웃 범위 안에서만 편하다고 느꼈다. 결국 표준적인 리스트/그리드는 검증된 라이브러리를 쓰고, masonry에 커스텀 배치하는 것과 같이 라이브러리가 다루지 못하는 특수한 요구가 있을 때는 계산만 해주는 도구(TanStack) 위에 직접 배치를 얹거나 직접 구현하는 방향이 합리적이라고 판단했다. 그리고 무엇을 쓰든, 직접 한 번 만들어본 경험이 있어야 라이브러리가 내부에서 무슨 일을 하는지 이해하고 디버깅할 수 있다는 것도 이번에 느꼈다.
